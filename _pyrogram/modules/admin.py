 ...
 **Admin Tools** 
  ...
  `ban` -> __Bans user indefinitely__
  `unban` -> __Unbans the user__
  `promote` [optional title] -> Promotes a user.
  `demote` _> __Demotes a user__
  `mute` -> __Mutes user indefinitely__
  `unmute` -> __Unmutes the user__
  `kick` -> __Kicks the user out of the group__
  `gmute` -> __Doesn't lets a user speak(even admins)__
  `ungmute` -> __Inverse of what gmute does__
  `pin` -> __pins a message__
  `del` -> __delete a message__
  `purge` -> __purge message(s)__
  `invite` -> __add user to chat__
"""
    }
)
...
async def ban_hammer(_, message: Message):
    ...
    try:
        ...
        await message.edit(f"Go Away [{get_user.first_name}](tg://user?id={get_user.id}) Stupid... I dont Like your Attittude.. Get List 😤.")
...
async def unban(_, message: Message):
    ...
    try:
        ...
        await message.edit(f"Welcome Back [{get_user.first_name}](tg://user?id={get_user.id})... Successfully Unbanned from the chat.")
    except Exception as e:
        await message.edit(f"{e}")
...
async def unmute(_, message: Message):
    ...
    try:
        ...
        await message.edit(f"[{get_user.first_name}](tg://user?id={get_user.id}) You Shut Up 🤐**")
    except Exception as e:
        await message.edit(f"{e}")
...
async def kick_usr(_, message: Message):
    ...
    try:
        ...
        await app.unban_chat_member(chat_id=message.chat.id, user_id=get_user.id)
        await message.edit(f"Get Lost [{get_user.first_name}](tg://user?id={get_user.id})")
    except Exception as e:
        await message.edit(f"{e}")
